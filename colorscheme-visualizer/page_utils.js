		...
		data.forEach(rowDataArray => {
			...
			for (let i = 0; i < numCols; i++) {
				...
				if (cellName !== undefined && cellName !== null && typeof cellName === 'string' && cellName.trim() !== "") {
					...
					cell.dataset.name = cellName; // Store name in data attribute
...
function cellColor(cellName, hexColor) {
	...
	if (cell && hexColor) {
		...
		if (!rgb) {
			console.warn(`Invalid hex color provided: "${hexColor}" for cell "${cellName}"`);
			return;
		}

		cell.style.backgroundColor = normalizedHex;
		cell.style.color = getContrastingTextColor(normalizedHex);

		const r_norm = (rgb.r / 255).toFixed(3);
		const g_norm = (rgb.g / 255).toFixed(3);
		const b_norm = (rgb.b / 255).toFixed(3);

		cell.innerHTML = `<div class="cell-name">${cellName}</div><div class="color-info">${normalizedHex}<br>{${r_norm}, ${g_norm}, ${b_norm}}</div>`;
		cellColorRegistry[cellName] = normalizedHex;

	} else if (cell && !hexColor) { // Reset color
		cell.style.backgroundColor = '';
		cell.style.color = '';
		cell.innerHTML = cellName; // Reset content to just the name
			...
			for (let r2 = 0; r2 <= rowCount - 1; r2++) {
				for (let c2 = 0; c2 <= colCount - 1; c2++) {
					...
					const cellData = cellsInSquare.flat().map(cellDOM => {
						const name = cellDOM.dataset.name;
						return {
							dom: cellDOM,
							name: name,
							color: name ? getCellColor(name) : null,
							hasText: !!name
						};
					});
